"""Fetch NBA or WNBA game-log endpoints and upsert into the correct ``raw.*`` tables.

One endpoint → one DataFrame → one ``raw`` (or ``raw.wnba_*``) table. No merging
here; use ``src.utils.silver`` / ``src.utils.wnba_silver`` for flat modeling frames.

Examples::

    python src/utils/playerLogs.py --league nba --season 2025-26 --db-upsert
    python src/utils/playerLogs.py --league wnba --season 2025 --db-upsert
"""
...
# Dataset keys (in-memory / CLI) shared by both leagues.
...
# BoxScorePlayerTrackV3 camelCase → SCREAMING_SNAKE.
...
# Legacy abbreviated names for downstream merge / CSV pipelines.
...
LEAGUES: dict[LeagueKey, LeagueConfig] = {
    'nba': LeagueConfig(
        ...
        default_checkpoint='start_positions_checkpoint.csv',
    ),
    'wnba': LeagueConfig(
        key='wnba',
        label='WNBA',
...
# Backwards-compat export used by wnba_silver.py.
RAW_TABLE_BY_DATASET = LEAGUES['wnba'].raw_table_by_dataset
...
def _upsert_raw(table: str, df: pd.DataFrame) -> None:
    try:
        ...
        if table in ('start_positions', 'wnba_start_positions'):
...
def refresh_tracking_checkpoint(
    ...
    *,
    ...
    cfg = LEAGUES[league]
    if league != 'nba':
        raise ValueError("refresh_tracking_checkpoint is only supported for NBA")
    ...
    if not remaining:
        ...
        if db_upsert:
            _upsert_raw(cfg.raw_table_by_dataset['start_positions'], result)
        return result
...
class GameLogs:
    ...
    def fetch(
        ...
        *,
        ...
        """Fetch one or more endpoints and optionally upsert into Supabase raw tables."""
...
class NBAGameLogs(GameLogs):
    """Backwards-compatible NBA fetcher."""
...
class WNBAGameLogs(GameLogs):
    """Backwards-compatible WNBA fetcher."""
...
def _parse_cli() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Fetch NBA or WNBA game-log endpoints into Supabase raw tables.",
    )
    p.add_argument(
        '--league',
        choices=('nba', 'wnba'),
    ...
    p.add_argument('--season', default=None, help='Season label (default: league-specific)')
    ...
    p.add_argument('--checkpoint', default=None, help='start_positions checkpoint path')
...
def main() -> int:
    ...
    season = args.season or cfg.default_season
