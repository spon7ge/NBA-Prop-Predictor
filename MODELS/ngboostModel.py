...
def optimize_score_dependent_calibration(mean_model, variance_model, val_df, features, target_col,
                                         base_factor=1.25, prediction_type='median',
                                         optimize_for='q75'):
    """Optimize score-dependent calibration parameters to improve Q75 prediction.
    ...
    Args:
        ...
        features: List of feature names
        target_col: Target column name
        base_factor: Base calibration factor
        prediction_type: 'mean', 'median', or 'robust_mean'
        optimize_for: Which metric to optimize ('q75', 'q50', 'all')
    ...
    # Get predicted means in original space for threshold determination
    target_transform = getattr(mean_model, 'target_transform_', None)
    if target_transform == 'log':
    ...
    def objective(params):
        ...
        if optimize_for == 'q75':
            q75_pred = norm.ppf(0.75, loc=mu, scale=sigma).mean()
            q75_actual = np.percentile(y_actual, 75)
            error = abs(q75_pred - q75_actual)
        elif optimize_for == 'q50':
            q50_pred = norm.ppf(0.5, loc=mu, scale=sigma).mean()
            q50_actual = np.percentile(y_actual, 50)
            error = abs(q50_pred - q50_actual)
        else:  # 'all'
            errors = []
            for q in [0.25, 0.5, 0.75]:
                q_pred = norm.ppf(q, loc=mu, scale=sigma).mean()
                q_actual = np.percentile(y_actual, q * 100)
                errors.append(abs(q_pred - q_actual))
            error = np.mean(errors)
        
        return error
    
    # Initial guess: thresholds at 10 and 15, factors increasing
    initial_params = [8.0, 15.0, 0.9, 1.0, 1.4]
    
    # Bounds: [low_thresh, high_thresh, low_fac, mid_fac, high_fac]
    bounds = [
        (5.0, 12.0),   # low_score_threshold
        (12.0, 25.0),  # high_score_threshold (increased upper bound)
        (0.7, 1.1),    # low_factor
        (0.9, 1.3),    # mid_factor
        (1.2, 2.5)     # high_factor (increased for very high scorers)
    ]
    ...
    if best_result is not None and best_result.success:
        ...
        print("Try manual tuning based on score range analysis:")
        print("  - Very High (25+): Q75 error -1.9 (needs more variance)")
        print("  - High (16-25): Q75 error -0.6")
        print("  - Mid-High (12-16): Q75 error +0.1 (good!)")
        return {
            'low_score_threshold': 10.0,
            'high_score_threshold': 20.0,  # Higher threshold - only apply high_factor to very high scorers
            'low_factor': 1.1,  # Slightly increase for low scorers
            'mid_factor': 1.0,   # Keep mid range at base
            'high_factor': 1.8   # Increased for very high scorers (25+)
        }
