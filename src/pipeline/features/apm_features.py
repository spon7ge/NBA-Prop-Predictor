...
def apm_features(df: pd.DataFrame) -> pd.DataFrame:
    """Leakage-safe feature pipeline for the APM quantile model."""
    df = df.copy()
...
def _per_min_features(df: pd.DataFrame) -> pd.DataFrame:
    per_min_map = {
        "AST_PER_MIN": ("AST", "MIN"),
        "TOV_PER_MIN": ("TOV", "MIN"),
        "TCHS_PER_MIN": ("TCHS", "MIN"),
        "SAST_PER_MIN": ("SAST", "MIN"),
        "FTAST_PER_MIN": ("FTAST", "MIN"),
        "PASS_PER_MIN": ("PASS", "MIN"),
        "POSS_PER_MIN": ("POSS", "MIN"),
    }
...
def _quantile_model_features(df: pd.DataFrame) -> pd.DataFrame:
    if "AST_PER_MIN_roll10" in df.columns:
        df["TEAM_AST_PER_MIN_RANK_L10"] = (
            df.groupby(["TEAM_ID", "GAME_DATE"])["AST_PER_MIN_roll10"]
            .rank(ascending=False, method="dense")
        )
    if "AST_PER_MIN_5_ewm" in df.columns and "AST_PER_MIN_season_avg" in df.columns:
        df["APM_TREND"] = df["AST_PER_MIN_5_ewm"] - df["AST_PER_MIN_season_avg"]
    if "AST_PER_MIN" in df.columns:
        df["AST_PER_MIN_STD_SEASON"] = (
            df.groupby("PLAYER_ID")["AST_PER_MIN"]
            .transform(lambda x: x.shift(1).expanding(min_periods=2).std())
        )
    if "POS" in df.columns:
        df["POSITION_ENCODED"] = pd.Categorical(df["POS"]).codes
    return df
