...
def rollingAverages(player_data, player_id_col='PLAYER_ID', date_col='GAME_DATE', windows=[10,15,25,40]):
    ...
    stats_cols = [ 
    'MIN', 'REB', 'OREB', 'DREB', 'BLK', 'PIE', 'PLUS_MINUS', 'FANTASY_PTS', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'ORBC', 'DRBC', 'RBC',
    'PTS_PAINT', 'E_OFF_RATING', 'E_DEF_RATING', 'SPD', 'DIST', 'POSS'
]
...
def getPlayerAvgToDateVectorized(df, player_id_col='PLAYER_ID', date_col='GAME_DATE'):
    ...
    stats_cols = [ 
    'MIN', 'REB', 'OREB', 'DREB', 'BLK', 'PIE', 'PLUS_MINUS', 'FANTASY_PTS', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'ORBC', 'DRBC', 'RBC',
    'PTS_PAINT', 'E_OFF_RATING', 'E_DEF_RATING', 'POSS'
]
...
def HomeAwayAverages(player_data, player_id_col='PLAYER_ID', date_col='GAME_DATE'):
    ...
    metrics = [ 
    'MIN', 'REB', 'OREB', 'DREB', 'BLK', 'PIE', 'PLUS_MINUS', 'FANTASY_PTS', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'ORBC', 'DRBC', 'RBC',
    'PTS_PAINT', 'E_OFF_RATING', 'E_DEF_RATING', 'SPD', 'DIST'
]
...
def statAgainstTeam(player_data, player_id_col='PLAYER_ID', opp_col='OPP_ABBREVIATION', stat_line='PTS'):
    ...
    metrics = {
        ...
        'RBC': [3,5,10,15],
        'PTS_PAINT': [3,5,10,15],
        'E_OFF_RATING': [3,5,10,15],
        'E_DEF_RATING': [3,5,10,15],
...
def teamContext(df):
    ...
    df['TEAM_OREB_AVG_TO_DATE'] = df.groupby('TEAM_ID')['TEAM_OREB'].transform(
        lambda x: x.shift(1).expanding().mean().round(2)
    )
    df['TEAM_DREB_AVG_TO_DATE'] = df.groupby('TEAM_ID')['TEAM_DREB'].transform(
        lambda x: x.shift(1).expanding().mean().round(2)
    )
    return df
...
# ================================================================================================
def processBestReboundersData(df, min_minutes=10):
    df = df.copy()
    ...
    for stat in ['REB', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'ORBC', 'DRBC', 'RBC', 'PTS_PAINT', 'E_OFF_RATING', 'E_DEF_RATING']:
    ...
    normalized_stats['BEST_REBOUNDER'] = (
        0.25 * normalized_stats['REB_NORM'] +     
        0.20 * normalized_stats['OREB_PCT_NORM'] +       
        0.15 * normalized_stats['DREB_PCT_NORM'] +      
        0.20 * normalized_stats['PTS_PAINT_NORM'] +          
        0.15 * normalized_stats['E_OFF_RATING_NORM'] +         
        0.05 * normalized_stats['E_DEF_RATING_NORM'] +    
        0.15 * normalized_stats['ORBC_NORM'] +          
        0.15 * normalized_stats['DRBC_NORM'] +          
        0.15 * normalized_stats['RBC_NORM']           
    )
    
    # Select highest scoring player per team as star
    star_rows = (
        normalized_stats.sort_values(['TEAM_ID', 'BEST_REBOUNDER'], ascending=[True, False])
    ...
    df['BEST_REBOUNDER_NAME'] = df['TEAM_ID'].map(star_by_team)
    df['PLAYER_IS_TEAM_BEST_REBOUNDER'] = (df['PLAYER_NAME'] == df['BEST_REBOUNDER_NAME']).astype(int)

    # Star active status per game
    star_active_per_game = (
        df[df['PLAYER_NAME'] == df['BEST_REBOUNDER_NAME']]
        .groupby(['GAME_ID', 'TEAM_ID'], as_index=False)['ACTIVE']
        .max()
        .rename(columns={'ACTIVE': 'BEST_REBOUNDER_ACTIVE'})
    )
    df = df.merge(star_active_per_game, on=['GAME_ID', 'TEAM_ID'], how='left')
    df['BEST_REBOUNDER_ACTIVE'] = df['BEST_REBOUNDER_ACTIVE'].fillna(0).astype(int)

    # TEAM_STAR_OUT for non-star rows only
    df['TEAM_STAR_OUT'] = ((df['PLAYER_IS_TEAM_BEST_REBOUNDER'] == 0) & (df['BEST_REBOUNDER_ACTIVE'] == 0)).astype(int)

    # Cleanup helpers
    df = df.drop(columns=['BEST_REBOUNDER_NAME', 'BEST_REBOUNDER_ACTIVE', 'ACTIVE'])
...
def add_performance_without_stars_columns(df, min_games=2):
    ...
    metrics = [ 
    'MIN', 'REB', 'OREB', 'DREB', 'BLK', 'PIE', 'PLUS_MINUS', 'FANTASY_PTS', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'ORBC', 'DRBC', 'RBC',
    'PTS_PAINT', 'E_OFF_RATING', 'E_DEF_RATING', 
]
    ...
    def calculate_without_star_stats(player_group):
        ...
        if star_out_mask.sum() >= min_games:
            ...
            rebs_shifted = star_out_data['REB'].shift(1)
            min_shifted = star_out_data['MIN'].shift(1)
            player_group['REB_PER_36_WITHOUT_STAR'] = round(
                (rebs_shifted * 36 / (min_shifted + 1e-8)).mean(), 2
            )
            ...
            player_group['REB_PER_36_WITHOUT_STAR'] = np.nan
...
def add_performance_volatility_categories(df, player_id_col='PLAYER_ID'):
    ...
    key_metrics = ['REB_EXPANDING_CV_TO_DATE', 'MIN_EXPANDING_CV_TO_DATE', 'USG_PCT_EXPANDING_CV_TO_DATE']
...
def add_recent_form_volatility(df, player_id_col='PLAYER_ID', date_col='GAME_DATE', lookback_games=5):
    ...
    key_stats = ['REB', 'OREB_PCT', 'DREB_PCT', 'REB_PCT', 'ORBC', 'DRBC', 'RBC', 'PTS_PAINT', 'E_OFF_RATING', 'E_DEF_RATING']
...
def add_interaction_features(df):
    ...
    df['REB_PER_MIN'] = round(df['REB_AVG_TO_DATE'] / (df['MIN_AVG_TO_DATE'] + eplison), 3)
    df['REB_PER_36'] = round(df['REB_AVG_TO_DATE'] * 36 / (df['MIN_AVG_TO_DATE'] + eplison), 3)
    df['REB_PER_40'] = round(df['REB_AVG_TO_DATE'] * 40 / (df['MIN_AVG_TO_DATE'] + eplison), 3)
    df['REB_PER_MIN_ROLLING_AVG_5'] = round(df['REB_ROLLING_AVG_5'] / (df['MIN_ROLLING_AVG_5'] + eplison), 3)
    df['REB_PER_MIN_ROLLING_AVG_15'] = round(df['REB_ROLLING_AVG_15'] / (df['MIN_ROLLING_AVG_15'] + eplison), 3)   
    df['REB_PER_MIN_ROLLING_AVG_40'] = round(df['REB_ROLLING_AVG_40'] / (df['MIN_ROLLING_AVG_40'] + eplison), 3)
    df['REB_PER_MIN_X_PACE'] = round(df['REB_PER_MIN'] * df['EXPECTED_PACE'], 3)
    df['REB_PER_MIN_X_POSS'] = round(df['REB_PER_MIN'] * df['POSS_AVG_TO_DATE'], 3)
    df['REB_PER_MIN_X_TEAM_OFF'] = round(df['REB_PER_MIN'] * df['TEAM_OFF_RATING_AVG_TO_DATE'], 3)
    df['REB_PER_MIN_X_OPP_DEF_RATING'] = round(df['REB_PER_MIN'] * df['OPP_DEF_RATING_AVG_TO_DATE'], 3)
    df['REB_PER_40_X_PACE'] = round(df['REB_PER_40'] * df['EXPECTED_PACE'], 3)
    df['REB_PER_40_X_POSS'] = round(df['REB_PER_40'] * df['POSS_AVG_TO_DATE'], 3)
    df['REB_PER_40_X_TEAM_OFF'] = round(df['REB_PER_40'] * df['TEAM_OFF_RATING_AVG_TO_DATE'], 3)
    df['REB_PER_40_X_OPP_DEF_RATING'] = round(df['REB_PER_40'] * df['OPP_DEF_RATING_AVG_TO_DATE'], 3)
