"""
NBA full-game spread + totals from Odds-API.io (v3) for FanDuel and DraftKings.

Default save dir: ``data/props/dk+fd_team_lines/`` (``dk+fd_YYYYMMDD_HHMMSS.json``).

Env: ``API_KEY_IO_1`` (or ``ODDS_API_IO_KEY``). Optional ``ODDS_API_IO_TIMEOUT``,
``ODDS_API_IO_RETRIES``.
"""
...
def _resolve_api_key(explicit: str | None) -> str:
    ...
    if not key:
        raise ValueError(
            "Set API_KEY_IO_1 (or ODDS_API_IO_KEY) or pass api_key= to fetchTeamLines."
        )
    return key
...
def fetchTeamLines(
    ...
    """
    Fetch Spread + Totals for NBA (FanDuel, DraftKings); write JSON under ``save_dir``
    (default ``data/props/dk+fd_team_lines``).

    Returns ``(records, path)`` where ``records`` match the ``records`` array in the file.
    """
    try:
    ...
    file_path = root_dir / f"dk+fd_{date_s}_{time_s}.json"
