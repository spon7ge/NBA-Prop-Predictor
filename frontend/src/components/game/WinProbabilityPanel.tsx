    ...
    <section className="rounded-xl border border-white/10 bg-[#141414] p-4">
      ...
      {data.teamStats.length > 0 ? (
        <div className="mt-6 space-y-3">
          <div className="grid grid-cols-[40px_1fr_40px] items-center gap-3 text-xs text-white/40">
            <span>{detail.away.abbrev}</span>
            <span className="text-center">Team stats</span>
            <span className="text-right">{detail.home.abbrev}</span>
          </div>
          {data.teamStats.map((stat) => (
            <div
              key={stat.key}
              className="grid grid-cols-[40px_1fr_40px] items-center gap-3 text-sm"
            >
