...
- **Top Legs** — Shows ranked **parlays** by **expected value (EV)**. You pick a **book** (PrizePicks, Underdog, DraftKings & Betr); the page loads that book’s slate file and renders each parlay as a card with EV, hit probability, Kelly, both legs (lines, sides, model context, form, matchup notes), and basic game info (total, spread).
